...
(function (window, $, undefined) {
	...
	App.getToken = function () {
		...
		var match = document.cookie.match(/XSRF-TOKEN=(\w+)/);
	...
	App.modal = function (options) {
		...
		$('<button/>', {'class': 'btn btn-default', 'type': 'button', 'data-dismiss': 'modal', 'text': 'Cancel'}).
			appendTo($footer);
		...
		if (options.prepare) {
			options.prepare($body, $submit, options);
		}
	...
	App.require = function (namespaces, callback, asynchronous) {
		...
		(_.isArray(namespaces) ? namespaces : [namespaces]).forEach(function (namespace) {
			...
			if (typeof App.scriptMap[namespace] === 'undefined') {
				console.error('Namespace', namespace, ' not defined');
			}
	...
	App.refreshRequirements = function () {
		...
		this.require(namespaces, function () {
			console.log('Requirements loaded', namespaces);
		});
	};
	...
	App.registerDefaultScripts = function () {
		...
		this.registerScript('slug', [
			'vendor/speakingurl/lib/speakingurl.js',
			'js/app/slug.js'
		]);
