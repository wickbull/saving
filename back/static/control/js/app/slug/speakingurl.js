(function (root) {
    ...
    var getSlug = function getSlug(input, opts) {
        ...
        if (opts && opts.lang && window.getSlugLangPreprocess && window.getSlugLangPreprocess[opts.lang]) {
          var currentLangPreprocess = window.getSlugLangPreprocess[opts.lang];

          ['first', 'middle', 'last', 'other'].forEach(function (mode) {
            currentLangPreprocess[mode] && _.each(currentLangPreprocess[mode], function (tr, ntr) {
              var reg = ntr;

              if (mode === 'first') reg = '\b' + ntr;
              if (mode === 'middle') reg = '\B' + ntr + '\B';
              if (mode === 'last') reg = ntr + '\b';

              input = input.replace(new RegExp(reg, 'g'), tr);
            });
          });
        }
